...
!(function(e, t) {
	...
	return class {
		...
		prepareElements() {
			...
			window.removeEventListener('scroll', this.prepareElements),
				(this.elements = this.elements.filter(t => {
					...
					return s || (t.classList.add(n), t.classList.add(e)), !s;
				})),
		...
		callback(e) {
			const { elementVisibleClassName: t } = this.config;
			e.forEach(e => {
				const { target: s } = e;
				e.isIntersecting &&
					(s.classList.add(t),
		...
		unmountIf() {
			const { observedElements: e } = this.state;
				const { unmountOnRevealAll: t } = this.config;
			e === 0 && t && this.unmount();
		}
		...
		observe(e) {
			this.observer.observe(e), (this.state.observedElements += 1);
		}
	};
});
